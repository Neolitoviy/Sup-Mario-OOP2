...
class Platform:
    ...
    def spawn_bonus(self, core):
        ...
        if self.bonus == 'mushroom':
            core.get_sound().play('mushroom_appear', 0)
            ...
            core.get_sound().play('coin', 0)
            core.get_map().spawn_participles(self.rect.x + 8, self.rect.y - 32, 1)
            core.get_map().get_player().add_coins(1)
            core.get_map().get_player().add_score(200)

    def destroy(self, core):
        core.get_map().spawn_participles(self.rect.x, self.rect.y, 0)
    ...
    def render(self, core):
        ...
        elif self.typeID == BrickBlock and self.shaking:
            self.shake()
